...
static bool InitComposition(HWND hwnd, bool dark) {
    // Create dispatcher queue for the compositor thread
    ...
    // Get compositor from the desktop interop
    ComPtr<ICompositor> compositor;
    hr = desktopInterop.As(&compositor);
    if (FAILED(hr)) return false;

    // Create HostBackdropBrush (Mica/Acrylic)
    ...
    // Create a SpriteVisual and set the backdrop brush
...
bool AppWindow::create() {
    ...
    // Extend DWM frame for glass effect
    MARGINS margins{-1};
    DwmExtendFrameIntoClientArea(m_hwnd, &margins);

    // Also apply acrylic via SetWindowCompositionAttribute (Win10 fallback)
    ...
    if (hUser32) {
        ...
        if (fn) {
            ...
            p.State = 4;
            p.Color = dark ? 0x80000000 : 0xB0FFFFFF;
            struct { int A; void* D; ULONG S; } d{19, &p, sizeof(p)};
            fn(m_hwnd, &d);
        }
    }

    // Use WinRT HostBackdropBrush for proper Mica/Acrylic on Win11
...
void AppWindow::toggle() {
    if (IsWindowVisible(m_hwnd)) hide(); else show();
}
    ...
    switch (msg) {
        ...
        case WM_NOTIFY:
            ...
            if (((NMHDR*)lp)->code == NM_DBLCLK && ((NMHDR*)lp)->idFrom == ID_LIST) {
                auto* nmia = (NMITEMACTIVATE*)lp;
                if (nmia->iItem >= 0 && nmia->iItem < (int)m_items.size()) copyToClipboard(nmia->iItem);
            }
            return 0;
        case WM_CONTEXTMENU:
            if ((HWND)wp == m_listView) { POINT pt = {GET_X_LPARAM(lp), GET_Y_LPARAM(lp)}; onContextMenu(pt); }
            return 0;
        case WM_ACTIVATE:
...
void AppWindow::onCreate() {
    m_closeBtn = CreateWindowExW(0, L"BUTTON", L"X", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
    ...
    m_searchBox = CreateWindowExW(0, L"EDIT", L"", WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL | WS_BORDER,
    ...
    m_listView = CreateWindowExW(0, WC_LISTVIEWW, L"", WS_CHILD | WS_VISIBLE | LVS_REPORT | LVS_SINGLESEL | LVS_NOCOLUMNHEADER,
...
void AppWindow::refreshList(const std::string& filter) {
    ...
    try {
        ...
        if (j["status"] != "ok") { SendMessageW(m_statusBar, SB_SETTEXT, 0, (LPARAM)L" Backend error"); return; }
        m_items.clear();
    ...
    } catch (...) { SendMessageW(m_statusBar, SB_SETTEXT, 0, (LPARAM)L" Backend not connected"); }
...
void AppWindow::updateListView(const std::vector<ClipItem>& items) {
    ...
    for (size_t i = 0; i < items.size(); i++) {
        ...
        std::wstring wDisplay = toWide(display); lvi.pszText = (LPWSTR)wDisplay.c_str();
...
void AppWindow::copyToClipboard(int index) {
    ...
    if (item.contentType == "text") {
        ...
        try {
            ...
            fileList += L'\0';
            size_t fb = fileList.size() * sizeof(wchar_t); const UINT HDR = 20;
            HGLOBAL hMem = GlobalAlloc(GMEM_MOVEABLE, HDR + fb);
            if (hMem) {
                BYTE* p = (BYTE*)GlobalLock(hMem); p[0]=20; p[1]=0; p[2]=0; p[3]=0;
                *(UINT*)(p+4)=0; *(UINT*)(p+8)=0; *(UINT*)(p+12)=0; *(UINT*)(p+16)=1;
                memcpy(p+HDR, fileList.c_str(), fb); GlobalUnlock(hMem);
                OpenClipboard(m_hwnd); EmptyClipboard(); SetClipboardData(CF_HDROP, hMem); CloseClipboard();
            }
        } catch (...) {}
    }
    hide();
...
void AppWindow::pinItem(int index) {
    if (index < 0 || index >= (int)m_items.size()) return;
    auto& item = m_items[index]; m_ipc.pinItem(item.id, item.pinned ? 0 : 1); item.pinned = !item.pinned;
    wchar_t text[256]; GetWindowTextW(m_searchBox, text, 256); refreshList(toNarrow(text));
}

void AppWindow::deleteItem(int index) {
    if (index < 0 || index >= (int)m_items.size()) return;
    m_ipc.deleteItem(m_items[index].id); m_items.erase(m_items.begin() + index); updateListView(m_items);
    wchar_t status[64]; swprintf_s(status, L" %zu items  |  Alt+, to toggle", m_items.size());
    SendMessageW(m_statusBar, SB_SETTEXT, 0, (LPARAM)status);
}

std::wstring AppWindow::toWide(const std::string& s) {
    if (s.empty()) return L""; int len = MultiByteToWideChar(CP_UTF8, 0, s.c_str(), -1, nullptr, 0);
    std::wstring r(len, L'\0'); MultiByteToWideChar(CP_UTF8, 0, s.c_str(), -1, &r[0], len); return r;
}
std::string AppWindow::toNarrow(const std::wstring& ws) {
    if (ws.empty()) return ""; int len = WideCharToMultiByte(CP_UTF8, 0, ws.c_str(), -1, nullptr, 0, nullptr, nullptr);
    std::string r(len, '\0'); WideCharToMultiByte(CP_UTF8, 0, ws.c_str(), -1, &r[0], len, nullptr, nullptr); return r;
}
std::string AppWindow::formatTimestamp(long long ts) {
    time_t t = (time_t)ts; struct tm tm_info; localtime_s(&tm_info, &t);
    char buf[32]; strftime(buf, sizeof(buf), "%Y-%m-%d %H:%M", &tm_info); return buf;
}
